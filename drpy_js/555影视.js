// 网址发布页 https://555lyn.com
var rule = {
	title: '555电影[V2]', // csp_AppYsV2
	host: 'https://wuwu553.online/',//取发布页
//	hostJs:'print(HOST);let html=request(HOST,{headers:{"User-Agent":PC_UA}});let src=jsp.pdfh(html,"#domainData&&data-info");src=base64Decode(src); print(src);HOST="https://"+JSON.parse(src).site_main+"/";',//获取第二个超链接作为HOST
	homeUrl:'/api.php/app/index_video',
	//homeUrl:'/api.php/app/video?tid=最新&class=&area=&lang=&year=&limit=20&pg=1',
	...
	class_url:'2&1&3&4',
	play_parse:true,
	lazy:'js:input=/ddvod/.test(input)?"http://jhsj.manduhu.com/?url="+input:input;input=input.split(",")[0];',
	limit:6,
	double:false,
	...
	var d=[];
	let jo1=jo.list.vlist;
	if (typeof jo1=='undefined'){
		jo1=jo.list;
	}
	jo1.forEach(function(it){
		
		if (typeof it.vlist=='undefined'){
			log(it)
			if (it.type_id!==124){
				d.push({
				title:it.vod_name,
			...
			if (it.type_id!==124){
				it.vlist.forEach((it1)=>{
					//if (it1.type_id!==124){
					d.push({
					title:it1.vod_name,
					img:it1.vod_pic,
					desc:it1.vod_remarks,
					url:it1.vod_id
					})
					//}
				});
