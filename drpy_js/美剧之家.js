...
let klists = [];
let ktab_urls = ktabs.map(it => HOST + pdfh(it, 'a&&href'));
ktab_urls.forEach((ktu) => {
    if (/javascript/.test(ktu)) {
    let klist = pdfa(request(ninput), '.jisu&&a').map(it => pdfh(it, 'a&&Text') + '$' + HOST + pdfh(it, 'a&&href'));
    klists.push(klist);           
    } else {
    let klist = pdfa(request(ktu), '.jisu&&a').map(it => pdfh(it, 'a&&Text') + '$' + HOST + pdfh(it, 'a&&href'));
    klists.push(klist);
    }
});
