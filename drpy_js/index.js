...
async function main() {
    ...
    let xbpq_files = pathLib.readDir(pathLib.join(path_dir, xbpq_path));
    // console.log(xbpq_files);
    ...
    js_files.filter(x=>!x.endsWith(".bak")).forEach((it, index) => {
    ...
    xbpq_files.filter(x=>!x.endsWith(".bak")).forEach((it, index) => {
        let rname = it.replace('.json', '');
        let extras = [''];
        let excludes = [];
        if (!excludes.includes(rname)) {
            extras.forEach((extra, index) => {
                ...
                let data = {
                    'key': extras.length > 1 ? `xbpq_json_${rname}${index}` : `xbpq_js_${rname}`,
                    'name': extras.length > 1 ? `${rname}(${ext_str})` : `${rname}(XBPQ)`,
                    'type': 3,
                    'api': "csp_XBPQ",
                    "jar": "./lib/XBPQ.jar",
                    'searchable': 1,
